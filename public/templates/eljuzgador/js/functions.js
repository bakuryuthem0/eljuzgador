...
function validate() {
	...
	$('.validate-input').each(function(index, el) {
		switch($(el))
		{
			case $(el).attr('type') == "text": 
				if (checkEmpty($(el)) == 0) {
					emptyMsg($(el));
					proceed = 0;
				}
				break;
			case $(el).attr('type') == "email":
				if(validateEmail($(el)))
				{
					emptyMsg($el);
					proceed = 0;
...
function cloneDouble(target, name1, name2, input1, input2) {
	var toClone = $('.'+target);
	var cloned = toClone.clone();
	toClone.removeClass(target).addClass('active');
	toClone.children('div').children(input1).attr('name',name1);
	toClone.children('div').children(input2).attr('name',name2);
	toClone.after(cloned);
}
